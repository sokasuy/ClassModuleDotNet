...
Public Class CStringManipulation
    ...
    Public Function GetSHA256Hash(_strToHash As String) As String
        'OK
        Try
            Dim sha256Obj As New Security.Cryptography.SHA256Managed()
            Dim bytesToHash() As Byte = System.Text.Encoding.UTF8.GetBytes(_strToHash)
            ...
            Return strResult
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "GetSHA256Hash Error")
            Return Nothing
        End Try
    End Function

    Public Function GetBcryptHash(_strToHash As String) As String
        'OK
        Try
            Dim strResult As String = BCrypt.Net.BCrypt.HashPassword(_strToHash, BCrypt.Net.BCrypt.GenerateSalt(12, "y"))
            Return strResult
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "GetBcryptHash Error")
            Return Nothing
        End Try
    End Function

    Public Function VerifyBcrypt(_strText As String, _strHash As String) As String
        'OK
        Try
            Dim strResult As String = BCrypt.Net.BCrypt.Verify(_strText, _strHash)
            Return strResult
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "VerifyBcrypt Error")
            Return Nothing
        End Try
    End Function
    ...
    Public Sub ValidateTextBox(ByRef _textBox As TextBox, _tbName As String, Optional _formatNumber As String = "#,##0.00", Optional _decimalDigit As Byte = 2)
        'OK
        'BISA KOMA
        Try
            Dim temp As String
            ...
            If (temp <> CleanInputDouble(Trim(_textBox.Text))) Then
                ...
                If Not (_textBox.Text.Length = 0) Then
                    ...
                    tamp = tamp * 10
                    tamp = Math.Round(tamp * 10.0, _decimalDigit) / 10
                    tamp = tamp / 10
                    'tamp = -Int(-(tamp))
    ...
    Public Function ConvertDataTableToString(_dt As DataTable, _colsGrouping As String) As String
        Try
            Dim sResult As String = ""
            Dim rdm As New Random()
            ...
            For i As Integer = 0 To _dt.Rows.Count - 1
                If (i > 0) Then
                    sResult &= ","
                End If
                sResult &= "'" & _dt.Rows(i).Item(_colsGrouping) & "'"
            Next

            Return sResult
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "ConvertDataTableToString Error")
            Return Nothing
        End Try
    End Function
End Class
